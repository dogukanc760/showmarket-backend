...
router.post("/register", async (req, res)=>{
    ...
    try {
        ...
        res.status(201).json({success:true, data:savedUser, message:"Register Successfully"});
    } catch (error) {
...
router.post("/login", async (req, res) => {
    try {
      ...
      if (Originalpassword !== req.body.password) {
        ...
        res.status(200).json({data:{...others, accessToken}, success:true, message:"Login Successfully"});
      }
    } catch (error) {
      res.status(500).json({success:false, message:error});
    }
